	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			if oldpassword!=user.Password:
				...
				return render(request,'CragoApp/CustomerChangePassword.html',{'form':form,'error':msg,'user':user})
	else:
...
def UserFeedback(request,id):
	if request.method=="POST":
		Feeback=FeedbackForm(request.POST,request.FILES)
		if Feeback.is_valid():
			ftb=FeebackTb()
			ftb.Comments=Feeback.cleaned_data['Comments']
			ftb.Questions=Feeback.cleaned_data['Questions']
			ftb.Describe_Feeback=Feeback.cleaned_data['Describe_Feeback']
			ftb.Name=Feeback.cleaned_data['Name']
			ftb.Lastname=Feeback.cleaned_data['Lastname']
			ftb.Email=Feeback.cleaned_data['Email']
			ftb.save()
		else:
			print(Feeback.errors)
			return render(request,'CragoApp/Feeback.html',{'data':Feeback})
	else:
		UserFeedback=FeedbackForm()
		return render(request,'CragoApp/Feedback.html',{'data':UserFeedback})
...
def CancelOrder(request,Cargotb_id):
	uid=request.session['id']
	user=BookingTB.objects.filter(Cargotb_id=uid)
	BookingTB.Status=1
	...
	if request.method == 'POST':
		user.delete()
		messages.success(request,'CancelOrder Succesfully..')
		return redirect('CargoApp/Orders.html')
	return render(request,'CargoApp/Orders.html',{'user':user,'uid':uid})
